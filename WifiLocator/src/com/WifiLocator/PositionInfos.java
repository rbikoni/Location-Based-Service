...
/**
 * In der Datenbank: Spots mit gleich Position_Id<br/>
...
public class PositionInfos {
		...
		for (int index = 0; index < len; ++index) {
			...
			for (int index2 = index + 1; index2 < len; ++index2) {
				...
				if (best.getMedian() < challenger.getMedian()) {
					...
					Log.v("PositionInfo:sort()", "Compared: "
							+ challenger.BSSID + "|" + challenger.getMedian()
							+ " " + loser.BSSID + "|" + loser.getMedian());
	...
	public void printPositionInfos(final TextView view) {
		...
		while (it.hasNext()) {
			...
			// float avgLevel = pi.calculateAvgLevel();
	...
	public String[] getBSSIDs() {
		...
		for (int index = 0; index < len; ++index) {
			...
			Log.v("getBSSIDs()", "" + index + ":" + bssids[index]);
		}
